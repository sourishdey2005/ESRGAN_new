...
# ----------------------------
# Safe Config for TF Memory
# ----------------------------
os.environ["TF_FORCE_GPU_ALLOW_GROWTH"] = "true"
tf.config.optimizer.set_experimental_options({"auto_mixed_precision": True})

st.set_page_config(layout="centered")
st.title("🧠 ESRGAN + Ultra-HD Output (Optimized)")
st.markdown("Upload **two low-res images** → ESRGAN (1x) → Final upscaled 4K/8K image with sharpening.")
...
uploaded_files = st.file_uploader("📥 Upload two low-res images", type=["png", "jpg", "jpeg"], accept_multiple_files=True)
res_choice = st.selectbox("🎯 Final Resolution", ["4K", "8K"], index=0)
...
if uploaded_files and len(uploaded_files) == 2:
    ...
    st.image(tensor_to_pil(fused), caption="🔄 Fused Input", use_container_width=True)
    ...
    base_output = tensor_to_pil(sr)

    del sr, fused, img1, img2
    gc.collect()

    with st.spinner("📈 Upscaling to final resolution and enhancing..."):
        upscale_img = upscale_to_resolution(base_output, res_choice)
        final_img = sharpen_image(upscale_img)

    tabs = st.tabs(["📈 ESRGAN Output", f"🏞️ Final {res_choice} Enhanced"])

    with tabs[0]:
        st.image(base_output, caption="🧠 ESRGAN Output (1x)", use_container_width=True)

    with tabs[1]:
        st.image(final_img, caption=f"{res_choice} + Sharpness", use_container_width=True)
    ...
    st.markdown(f"⏱️ **Time Taken:** `{t1 - t0:.2f}` seconds")

    buf = io.BytesIO()
    final_img.save(buf, format="PNG")
    st.download_button("⬇️ Download Final Image", buf.getvalue(), f"superres_{res_choice.lower()}.png", "image/png")

    st.success(f"✅ Done! {res_choice} Image Ready.")
else:
